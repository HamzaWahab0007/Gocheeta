...
<div class="container-fluid">
    <form action="" id="booking-form">
        ...
        <input type="hidden" name="cab_id"
            value="<?= isset($_GET['cid']) ? $_GET['cid'] : (isset($cab_id) ? $cab_id : "") ?>">
        <?php
                $query_meet = "SELECT * FROM region";
                $rows =  $conn->query($query_meet); 
        ...
        <div class="form-group">
            ...
            <textarea name="pickup_zone" id="pickup_zone" rows="4" class="form-control form-control-sm rounded-0"
                required></textarea>
        </div>
        <div class="form-group">
            <label for="drop_zone" class="control-label">Drop-off Location</label>
            <textarea name="drop_zone" id="drop_zone" rows="4" class="form-control form-control-sm rounded-0"
                required></textarea>
        </div>
    </form>
</div>
